...
############################################################################
# Operators: Sel: Tournament, Crossover: OnePoint, Mutation: Uniform
############################################################################
...
transitions_offline = 5 * transitions
...
if mutstr == "semantic":
    ...
    toolbox.register("expr_mut", gp.genGrow, min_=params_gp["mut_min"], max_=params_gp["mut_max"])
...
def main():
    ...
    if not offline_bool:
        ...
        train(use_run=params_gp["runs_offline"], run=1)
